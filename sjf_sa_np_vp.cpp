// C++ Program to demonstrate the working of Shortest Job First Scheduling Algorithm
...
//							time = p[m].ft = ps[i].ft = ps[i].st + ps[i].bt;
		...
		{
			for(k=i+1 ; k<=n && ps[k].at <= time ; k++ )			// sorting the remaining process according to their min burst time
			{
